...
public class MainActivity extends Activity {
 ...
 class Game extends View {
  ...
  Game(){super(MainActivity.this);setLayerType(LAYER_TYPE_SOFTWARE,null);background=BitmapFactory.decodeResource(getResources(),R.drawable.game_board_bg);newGame();}
  ...
  void peg(Canvas c,float x,float y,float r,boolean hit){int base=hit?0xffdf641d:0xfff1f2ef;p.setShader(new RadialGradient(x-r*.32f,y-r*.38f,r*1.25f,new int[]{Color.WHITE,base,mix(base,Color.BLACK,.35f)},new float[]{0,.45f,1},Shader.TileMode.CLAMP));p.setStyle(Paint.Style.FILL);p.setShadowLayer(r*.28f,0,r*.2f,0xaa000000);c.drawCircle(x,y,r,p);p.clearShadowLayer();p.setShader(null);}
  ...
  @Override protected void onDraw(Canvas c){float w=getWidth(),h=getHeight();
   c.drawBitmap(background,null,new RectF(0,0,w,h),p);
   float m=w*.035f,top=h*.07f;peg(c,w*.205f,top-h*.01f,h*.018f,true);text(c,"ヒット：色と場所が正解",w*.31f,top,h*.034f,Color.WHITE,Paint.Align.CENTER);peg(c,w*.49f,top-h*.01f,h*.018f,false);text(c,"ブロー：色だけ正解",w*.61f,top,h*.034f,Color.WHITE,Paint.Align.CENTER);
   float left=w*.115f,right=w*.82f;colW=(right-left)/8f;slotY0=h*.40f;slotDy=h*.135f;
   for(int col=0;col<8;col++){float cx=left+colW*(col+.5f);if(col<7){p.setColor(0xffc3cac8);p.setStyle(Paint.Style.FILL);path.reset();path.moveTo(cx+colW*.55f,h*.178f);path.lineTo(cx+colW*.38f,h*.155f);path.lineTo(cx+colW*.38f,h*.201f);path.close();c.drawPath(path,p);}avatar(c,cx,h*.157f,h*.036f,col);
    ...
    if(col<marks.size()){int hit=marks.get(col)[0],blow=marks.get(col)[1],n=0,show=col==markColumn?(int)(pinT*4.01f):4;for(int k=0;k<hit;k++,n++)if(n<show)peg(c,cx+(n%2-.5f)*colW*.25f,h*.247f+(n/2)*h*.042f,h*.012f,true);for(int k=0;k<blow;k++,n++)if(n<show)peg(c,cx+(n%2-.5f)*colW*.25f,h*.247f+(n/2)*h*.042f,h*.012f,false);}
   }
   colX=left+colW*(tries.size()+.5f);
   float ax=w*.90f;RectF answerCase=new RectF(ax-colW*.38f,h*.33f,ax+colW*.38f,h*.85f);metal(c,answerCase,h*.018f);for(int i=0;i<4;i++){hole(c,ax,slotY0+i*slotDy,h*.03f);marble(c,ax,slotY0+i*slotDy,h*.033f,answer[i]);}float lidUp=lidT*(answerCase.height()+h*.08f);RectF lid=new RectF(answerCase.left-2,answerCase.top-5-lidUp,answerCase.right+2,answerCase.bottom+5-lidUp);rect(c,lid,h*.018f,0xff828b89);p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(3);p.setColor(0xffdce2e0);c.drawRoundRect(lid,h*.018f,h*.018f,p);
   ...
   if(win&&partyT>0&&partyT<.98f)for(int i=0;i<20;i++){float x=(i*83)%w,y=((i*.173f+partyT*1.3f)%1)*h;p.setColor(C[i%6]);p.setStyle(Paint.Style.FILL);c.save();c.rotate(i*29+partyT*360,x,y);c.drawRect(x-3,y-7,x+3,y+7,p);c.restore();}
  }
